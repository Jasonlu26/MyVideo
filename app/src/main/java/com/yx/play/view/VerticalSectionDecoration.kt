...
class VerticalSectionDecoration private constructor(private val mContext: Context) :
    ...
    override fun getItemOffsets(
        ...
        if (!mShowFirst && !mShowLast && childPosition >= itemCount - lastDividerOffset) {
        ...
        if (mSectionProvider == null) {
            outRect.bottom = mSizeProvider?.dividerSize(childPosition, parent) ?: 0
        } else {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until validChildCount) {
            ...
            if (!mShowLast && childPosition >= itemCount - lastDividerOffset) {
            ...
            if (mSectionProvider != null && mSectionSizeProvider != null) {

                if (previousSectionName != currentName) {
                    ...
                    val top =
                        (child.top - dividerHeight).toFloat()
                    val bottom = child.top.toFloat()
                    dividerDraw(
                        canvas = c,
                        position = childPosition,
    ...
    fun sectionDrawable(@DrawableRes id: Int): VerticalSectionDecoration {
        return sectionDrawable(ContextCompat.getDrawable(mContext, id))
    }

    fun sectionDrawable(drawable: Drawable?): VerticalSectionDecoration {
        return sectionDrawableProvider(object : SectionDrawableProvider {
            override fun sectionDrawableProvider(position: Int, parent: RecyclerView?): Drawable? {
                return drawable
            }
        })
    }
    ...
    fun showLast(showLast: Boolean): VerticalSectionDecoration {
        mShowLast = showLast
        return this
    }

    fun showFirst(showFirst: Boolean): VerticalSectionDecoration {
